...
## S-03 Licencias de las fuentes {#s-03}

| Fuente | Licencia | Verificada | Que se tomo |
|---|---|---|---|
...
que eso no convierte a `engine/` en obra derivada, esta en
ver docs/decisions/ADR-0033-de-donde-salen-las-reglas.md#d-0331.
...
| Campo de latencia del arbitro, leido en el Go y comprobado en partidas reales del soak | 2026-09-18 | `you.latency` es la ida y vuelta que midio el arbitro, en milisegundos enteros truncados, y no sirve para contar timeouts (ver docs/rules-parametros.md#r-20) |
...
- **La referencia de API de `docs.battlesnake.com` sigue sin consultarse.** Lo verificado
  contra el Go es el campo de latencia **del CLI**, que es el arbitro con el que se juega
  en local; la DoD de la fase 7 mide contra el motor de produccion, y que ambos coincidan
  no esta comprobado. Cerrar esto sin abrir esa pagina seria dar por verificada una fuente
  que nadie ha leido.
