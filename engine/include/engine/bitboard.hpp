...
template <int W, int H> class Bitboard {
...
public:
    ...
    constexpr void set(int index) noexcept {
        words_[static_cast<unsigned>(index) / 64U] |= Word{1}
                                                      << (static_cast<unsigned>(index) % 64U);
    }
    ...
    [[nodiscard]] constexpr bool any() const noexcept {
        return std::ranges::any_of(words_, [](const Word w) { return w != 0; });
    }
    ...
    rect(int min_x, int min_y, int max_x, int max_y) noexcept {
        Bitboard r;
...
private:
    ...
    [[nodiscard]] constexpr Bitboard shifted_up(int n) const noexcept {
        Bitboard r;
        const auto shift = static_cast<unsigned>(n);
        const unsigned inv = 64U - shift;
        for (int i = word_count - 1; i >= 0; --i) {
            Word v = words_[static_cast<unsigned>(i)] << shift;
            if (i > 0) {
                v |= words_[static_cast<unsigned>(i - 1)] >> inv;
            }
    ...
    // El tamaño se convierte explicitamente: GCC avisa de conversion con signo
    // cuando el parametro de plantilla es `int`, y el build de deploy usa GCC.
    std::array<Word, static_cast<std::size_t>(word_count)> words_{};
};
